---
title: "Analysis for Gressier et al., 2022"
...
color_group_arunachalam <- c("Control"="#4F4D4C",
                 "Severe" = "#982326",
                 "Moderate" = "#5D63A1"
)
```
...
## FeatureBoxPlot
Credit Aaron Wilk, https://github.com/ajwilk/COVID_scMultiome
...
Load Seurat PBMC reference data from https://doi.org/10.1016/j.cell.2021.04.048
...
```{r}
# DE_genes <- read.csv("signatures/Mouse_DEG_Signatures_EG.csv")
# DE_genes
```
...
# lengths(sig_list)
# 
# save(sig_list, file = "analysis_output/Mouse_DEG_Signatures_EG_human.RDS")
load("analysis_output/Mouse_DEG_Signatures_EG_human.RDS")
```
...
# 
# 
# save(interferome_myeloid_list_top200_human, file = "analysis_output/Interferome_myeloid_list_top200_human.RDS")
load("analysis_output/Interferome_myeloid_list_top200_human.RDS")
```
...
# Tcell_sig_list <- list()
# 
# Tcell_sig1 <- read.delim("signatures/COMP3_DE_Table.txt",
#                          stringsAsFactors = F)
...
# }
# 
# save(Tcell_sig_list, file = "analysis_output/T_cell_signatures_Ahrends_human.RDS")
load("analysis_output/T_cell_signatures_Ahrends_human.RDS")
...
```{r}
seurat_cohort2_PBMC$group_time <- paste(seurat_cohort2_PBMC$group_per_sample, seurat_cohort2_PBMC$disease_stage, sep="_")
```
...
```{r, fig.width=7, fig.height=6}
 DimPlot(object = seurat_schulte_cohort2_pbmc,
  ...
  scale_color_manual(values = c("control"="#4f4d4c",
                         "mild (3)" ="#ffb867",
                         "moderate (4-5)" = "#5D63A1",
                         "severe (7)" = "#982326"))
```
...
# save(cells_rankings, file = "analysis_output/20230111_aucell_ranking_schulte_cohort2_PBMC.Robj")
```

```{r}
load("analysis_output/20230111_aucell_ranking_schulte_cohort2_PBMC.Robj")
```
...
## AUCell Enrichment analysis with cluster resoluition (Figure 6E)

```{r}
tmp <- as.matrix(seurat_schulte_cohort2_CD8_filtVarFeatures@assays$RNA@data)

rSum<-rowSums(tmp)
keep<-names(rSum[rSum>10])
tmp <- tmp[row.names(tmp) %in% keep,]
cells_rankings_cohort2_CD8 <- AUCell_buildRankings(as.matrix(tmp), verbose = TRUE, nCores=4)
```

```{r}
# Calculate AUC scores
cells_AUC_Tcell_help <- AUCell_calcAUC(Tcell_sig_list, 
                                       cells_rankings_cohort2_CD8, 
                                       aucMaxRank = ceiling(0.03 * nrow(cells_rankings_cohort2_CD8)), 
                                       normAUC = T)
```
...
df <- df[df$Row.names %in% Cells(seurat_schulte_cohort2_CD8_filtVarFeatures),]
...
## Subset analysis of myeloid cells as annotated in Schulte-Schrepping et al., Cell 2020
...
# Severity-specific DE genes (Suppl. Table 5, sheet 1)
...
###### HALLMARK (Suppl. Table 5, sheet 2)
...
seurat_schulte_cohort2_Mono$severity_stage <- paste(seurat_schulte_cohort2_Mono$disease_severity,seurat_schulte_cohort2_Mono$disease_stage,sep="_")
seurat_schulte_cohort2_Mono$severity_stage <- factor(seurat_schulte_cohort2_Mono$severity_stage, levels=rev(c("severe (7)_late",
                                                                                                          "moderate (4-5)_late",
                                                                                                          "mild (3)_late",
                                                                                                          "severe (7)_early",
                                                                                                          "moderate (4-5)_early",
...
##### Construct TF network (Figure 6B)
...
## Intersect CD40 unresponsive, amplified and combinatorial signature genes with DEGenes (Figure 4C)
...
# Sample-specific GSVA (Figure 4B)
...
gsva.res <- as.data.frame(t(gsva(expr = expr_mtx, 
                                 gset.idx.list = list_compressedIFN[c("IFN","Amplified","DoubleTrigger")], 
                                 method = "gsva",
                                 verbose=FALSE)))
gsva.res$sampleID <- rownames(gsva.res)
...
```{r}
meta <- unique(seurat_schulte_cohort2_Mono@meta.data[,c("sampleID","disease_severity","disease_stage")])
idx <- match(gsva.res.melt$sampleID, meta$sampleID)
gsva.res.melt$disease_severity <- meta$disease_severity[idx]
gsva.res.melt$disease_stage <- meta$disease_stage[idx]
```
...
p <- ggplot(gsva.res.melt, aes(y =value, x = disease_severity)) +
  ...
  labs(color = "COVID19 severity", shape = "stage")+
  scale_color_manual(values=color_group_schulte)+
  theme_classic()+ylab("Enrichment score")
...
### Identify severity marker genes (Figure 4D, Suppl. Table 6)
...
# Analysis of additional data sets incl. patients with IFN-AABs (Suppl. Fig. 3 B/C)

# --------------------------------------------------------

# 10x 5' Data on IFN-AAB patients from Charité (Sawitzki group)

Data has been demultiplexed and quality controlled by Lev Petrov as described in the methods section of the manuscript.

```{r}
seurat_Sawitzki <- LoadH5Seurat("seurat_objects/seurat_sawitzki_221202.h5seurat")

seurat_Sawitzki
```

### Quality Control
Filtering by Lev Petrov:

n_feature_lower_filter <- 200
n_feature_upper_filter <- 5000
percent.mt_upper_filter <- 10
nCount_RNA_upper_filter <- 20000


```{r, fig.width= 8, fig.height=10}
p1<-VlnPlot(object = seurat_Sawitzki, features = "nFeature_RNA",assay = "RNA", pt.size = 0, log =TRUE, group.by = "Patient.Id") +
  theme(legend.position="none") + ggtitle("Genes per Cell") + 
  ylab("Number of Genes") + xlab("")+ geom_hline(yintercept=c(200,5000), linetype="dashed", color = "red", size=1)

p2<-VlnPlot(object = seurat_Sawitzki, features = "nCount_RNA", assay = "RNA", log =TRUE, pt.size = 0, group.by = "Patient.Id")+
  theme(legend.position="none") + ggtitle("Transcripts per Cell") + 
  ylab("Number of UMIs") + xlab("")+ geom_hline(yintercept=c(200,20000), linetype="dashed", color = "red", size=1)

p3<-VlnPlot(object = seurat_Sawitzki, features = "percent.mt",assay = "RNA", pt.size = 0, group.by = "Patient.Id")+ 
  theme(legend.position="none") + ggtitle("Percent of MT- counts") + 
  ylab("% of '^mt-' gene counts") + xlab("")+ geom_hline(yintercept=10, linetype="dashed", color = "red", size=1)
...
```{r}
## Normalization
seurat_Sawitzki <- NormalizeData(object = seurat_Sawitzki, normalization.method = "LogNormalize", scale.factor = 1e4)

## Define variable genes
seurat_Sawitzki <- FindVariableFeatures(object = seurat_Sawitzki,
                                      assay="RNA",
                                      selection.method = 'vst')

## Scaling
seurat_Sawitzki <- ScaleData(object = seurat_Sawitzki, assay = "RNA",features = rownames(seurat_Sawitzki))

## PCA
seurat_Sawitzki <- RunPCA(object = seurat_Sawitzki, features = VariableFeatures(object = seurat_Sawitzki), 
                        verbose = FALSE,
                        assay="RNA")

## UMAP
seurat_Sawitzki <- RunUMAP(seurat_Sawitzki, reduction.use = "pca", dims = 1:20, seed.use = 42)
```


```{r, fig.height=12, fig.width=16}
p1 <- DimPlot(object = seurat_Sawitzki,
              reduction = 'umap',
              label = FALSE,shuffle = T,
              group.by = "sample")

p2 <- DimPlot(object = seurat_Sawitzki,
              reduction = 'umap',
              label = FALSE,shuffle = T,
              group.by = "dataset")

p3 <- DimPlot(object = seurat_Sawitzki,
              reduction = 'umap',
              label = FALSE,shuffle = T,
              group.by = "disease_severity")

p4 <- DimPlot(object = seurat_Sawitzki,
              reduction = 'umap',
              label = FALSE,shuffle = T,
              group.by = "batch")
...
ggplot(seurat_Sawitzki@meta.data, aes(x = batch, fill=sample))+
  geom_bar()+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
Normalize query data using sc transform 
```{r}
seurat_Sawitzki.sc <- SCTransform(seurat_Sawitzki, verbose = FALSE, assay = "RNA")
```

```{r}
anchors <- FindTransferAnchors(
  reference = reference,
  query = seurat_Sawitzki.sc,
  normalization.method = "SCT",
  reference.reduction = "spca",
  dims = 1:50
)
...
```{r, fig.width=12, fig.height=6}
p1 = DimPlot(seurat_Sawitzki.sc, reduction = "ref.umap", group.by = "predicted.celltype.l1", label = TRUE, label.size = 3, repel = TRUE) + NoLegend()
p2 = DimPlot(seurat_Sawitzki.sc, reduction = "ref.umap", group.by = "predicted.celltype.l2", label = TRUE, label.size = 3 ,repel = TRUE) + NoLegend()

p1 + p2
```

```{r, fig.width=12, fig.height=3}
ggplot(seurat_Sawitzki.sc@meta.data, aes(x = predicted.celltype.l2, fill=predicted.celltype.l2))+
  geom_bar()+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
seurat_Sawitzki$predicted.celltype.l1 <- seurat_Sawitzki.sc$predicted.celltype.l1
seurat_Sawitzki$predicted.celltype.l1.score <- seurat_Sawitzki.sc$predicted.celltype.l1.score

seurat_Sawitzki$predicted.celltype.l2 <- seurat_Sawitzki.sc$predicted.celltype.l2
seurat_Sawitzki$predicted.celltype.l2.score <- seurat_Sawitzki.sc$predicted.celltype.l2.score
```

```{r, fig.height=4, fig.width=12}
ggplot(seurat_Sawitzki@meta.data, aes(x = sample, fill=predicted.celltype.l2))+
...
```{r}
DimPlot(object = seurat_Sawitzki,
        reduction = 'umap',
        label = FALSE,shuffle = T,
...
```{r}
seurat_Sawitzki_Mono <- subset(seurat_Sawitzki, subset= predicted.celltype.l2 %in% c("CD14 Mono"))
seurat_Sawitzki_Mono
```

```{r}
table(seurat_Sawitzki_Mono$sample, seurat_Sawitzki_Mono$disease_severity)
```

#### Sub clustering of monocytes
```{r}
## Define variable genes
seurat_Sawitzki_Mono <- FindVariableFeatures(object = seurat_Sawitzki_Mono,
                                           assay="RNA",
                                           selection.method = 'vst')

## Scaling
seurat_Sawitzki_Mono <- ScaleData(object = seurat_Sawitzki_Mono, assay = "RNA", features = rownames(seurat_Sawitzki_Mono))

## PCA
seurat_Sawitzki_Mono <- RunPCA(object = seurat_Sawitzki_Mono, features = VariableFeatures(object = seurat_Sawitzki_Mono), 
                             verbose = FALSE,
                             assay="RNA")

## UMAP
seurat_Sawitzki_Mono <- RunUMAP(seurat_Sawitzki_Mono, reduction.use = "pca", dims = 1:20, seed.use = 42)
```


```{r, fig.width= 7, fig.height= 6}
DimPlot(object = seurat_Sawitzki_Mono,
        reduction = 'umap',
        group.by="batch",
        label = TRUE) + scale_color_manual(values=color_clusters) + NoLegend()
```
...
```{r, fig.width=16,fig.height=20}
seurat_Sawitzki_Mono <- FindNeighbors(object = seurat_Sawitzki_Mono, dims = 1:10, reduction="pca", force.recalc = TRUE)
```

```{r}
seurat_Sawitzki_Mono <- FindClusters(object = seurat_Sawitzki_Mono, resolution = 0.2, algorithm = 1)
```
...
#### Cells per seurat cell type per cluster

```{r, fig.height=5, fig.width=12}
ggplot(seurat_Sawitzki_Mono@meta.data, aes(x = RNA_snn_res.0.2, fill=batch))+
  geom_bar(position="fill")+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
Idents(seurat_Sawitzki_Mono) <- "RNA_snn_res.0.2"
seurat_Sawitzki_Mono_cluster.markers.wilcox_res.0.2 <- FindAllMarkers(object = seurat_Sawitzki_Mono,
                                                                    only.pos = TRUE,
                                                                    min.pct = 0.2,
                                                                    logfc.threshold = 0.2,
                                                                    min.diff.pct = 0.1,
...
```{r, fig.width=8, fig.height=12}
top <- seurat_Sawitzki_Mono_cluster.markers.wilcox_res.0.2 %>% group_by(cluster) %>% top_n(n = 10, wt = abs(avg_log2FC))

p <- DotPlot(seurat_Sawitzki_Mono,
             group.by = "RNA_snn_res.0.2",
             features =c("CD14", unique(top$gene)))+
  scale_color_gradientn(colors = rev(RColorBrewer::brewer.pal(n =100, name = "RdBu")))+
  theme(axis.title = element_blank()) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  coord_flip()
...
### Filter Monocyte/T cell cluster
```{r}
seurat_Sawitzki_Mono_filt <- subset(seurat_Sawitzki_Mono, subset = RNA_snn_res.0.2 == "4", invert=TRUE)
table(seurat_Sawitzki_Mono_filt$RNA_snn_res.0.2)
```

### Save monocyte seurat object

```{r}
# SaveH5Seurat(
#   object= seurat_Sawitzki_Mono_filt,
#   file= paste("seurat_objects/seurat_Sawitzki_Mono_filt_",Sys.Date(),sep=""))

# seurat_Sawitzki_Mono_filt <- LoadH5Seurat("seurat_objects/seurat_Sawitzki_Mono_filt_2023-01-12.h5seurat")
```

### Marker Gene Expression (Suppl. Figure 3B, top panel)

```{r, fig.width=18,fig.height=9}
seurat_Sawitzki_Mono_filt$disease_severity <- factor(seurat_Sawitzki_Mono_filt$disease_severity,levels=c("control",
                                                                                                     "1-3",
                                                                                                     "6-8",
                                                                                                     "7-8_anti-IFN"))

p <- FeatureBoxPlot(seu=seurat_Sawitzki_Mono_filt, 
               by = "features", 
               assay = "RNA", 
               slot = "data", 
               exprs.function = "mean",
               features = c("HLA-DRA","HLA-DRB1","ISG15", "IFIT2", "CD83", "CCL4", "TNF"), 
               meta.include = c("disease_severity","disease_stage"), 
               group_by = "disease_severity", 
               shape_by = "disease_stage", 
               each.pt = "sample", 
               custom_fill_colors = c("control"="#4f4d4c",
                                      "1-3" = "#ffb867",
                                      "4-5"="#5D63A1",
                                      "6-8"="#982326",
                                      "7-8_anti-IFN"="#DA2746"),
               color_by = "disease_severity", 
               comparisons = list(),
               # comparisons = list(c("1-3","6-8"),
               #                    c("1-3", "7-8_anti-IFN"),
               #                    c("6-8", "7-8_anti-IFN")), 
               ncol = 7, 
               label = "p.format", 
               select.idents = NULL, 
               label.x = NA, pt.size = NA, facet.face = "bold.italic") + 
  ggtitle("Sawitzki Monocyte RNA expression")+
  theme(plot.title = element_text(size=12))+scale_shape_manual(values = c("control"=15,
                                                                          "early"=19,
                                                                          "late"=17))
...
# 10X 5', Georg et al. Cell 2021

PBMC scRNA-seq data was provided by Rosario Astaburuaga-García for https://doi.org/10.1016/j.cell.2021.12.040 after personal communication.

```{r}
seurat_georg_orig <- LoadH5Seurat("seurat_objects/seurat_georg_orig_221102.h5seurat")

seurat_georg_orig
```
```{r}
head(seurat_georg_orig@meta.data)
```

```{r}
table(seurat_georg_orig$phase,seurat_georg_orig$disease)

table(seurat_georg_orig$is_pbmc)
```

```{r}
seurat_georg_orig$disease_severity_orig <- seurat_georg_orig$disease_severity
```


```{r}
seurat_georg_orig$disease_severity <- ifelse(seurat_georg_orig$disease_severity_merge == "HBV", "HBV",
                                             ifelse(seurat_georg_orig$disease_severity_merge == "FLI", "FLI",
                                                    ifelse(as.character(seurat_georg_orig$maxWHO) %in% c(1,2,3), "1-3",
                                                           ifelse(as.character(seurat_georg_orig$maxWHO) %in% c(4,5), "4-5",
                                                                  ifelse(as.character(seurat_georg_orig$maxWHO) %in% c(6,7,8), "6-8",
                                                                         ifelse(seurat_georg_orig$disease_severity_merge == "HC", "control",
                                                                                "error"))))))

table(seurat_georg_orig$disease_severity,seurat_georg_orig$disease_severity_merge)

seurat_georg_orig$disease_severity <- factor(seurat_georg_orig$disease_severity, levels=c("control",
                                                                                          "1-3",
                                                                                          "4-5",
                                                                                          "6-8",
                                                                                          "FLI",
...
p1<-VlnPlot(object = seurat_georg_orig, features = "nFeature_RNA",pt.size = 0, log =TRUE, group.by = "sample") + theme(legend.position="none") + ggtitle("Genes per Cell") + ylab("Number of Genes") + xlab("")+ geom_hline(yintercept=100, linetype="dashed", color = "red", size=1)

p2<-VlnPlot(object = seurat_georg_orig, features = "nCount_RNA", log =TRUE, pt.size = 0, group.by = "sample")+ theme(legend.position="none") + ggtitle("Transcripts per Cell") + ylab("Number of UMIs") + xlab("")+ geom_hline(yintercept=100, linetype="dashed", color = "red", size=1)

p3<-VlnPlot(object = seurat_georg_orig, features = "percent.mt",pt.size = 0, group.by = "sample")+ theme(legend.position="none") + ggtitle("Percent of MT- counts") + ylab("% of '^mt-' gene counts") + xlab("")+ geom_hline(yintercept=5, linetype="dashed", color = "red", size=1)
...
```{r}
seurat_georg <- seurat_georg_orig
DefaultAssay(seurat_georg) <- "RNA"
seurat_georg@assays$integrated <- NULL
```

#### Exclude T cell batches

```{r}
seurat_georg <- subset(seurat_georg, subset = is_pbmc == TRUE)
```


```{r, fig.height=6, fig.width=8}
## Normalization
seurat_georg <- NormalizeData(object = seurat_georg, normalization.method = "LogNormalize", scale.factor = 1e4)

## Define variable genes
seurat_georg <- FindVariableFeatures(object = seurat_georg,
                             assay="RNA",
                             selection.method = 'vst')

## Scaling
seurat_georg <- ScaleData(object = seurat_georg, assay = "RNA")

## PCA
seurat_georg <- RunPCA(object = seurat_georg, features = VariableFeatures(object = seurat_georg), verbose = FALSE)

## UMAP
seurat_georg <- RunUMAP(seurat_georg, reduction.use = "pca", dims = 1:20, seed.use = 42)
```



```{r, fig.height=6, fig.width=8}
DimPlot(object = seurat_georg,
        reduction = 'umap',
        label = FALSE,shuffle = T,
        group.by = "disease")+
  scale_color_manual(values = c("CV"="#ffb867",
                                "HC"="#6d1323",
                                "FLI"="#27dabb",
                                "HBV"="#27da62"))
```
...
Normalize query data using sc transform 
```{r}
seurat_georg.sc <- SCTransform(seurat_georg, verbose = FALSE, assay = "RNA")
```

```{r}
anchors <- FindTransferAnchors(
  reference = reference,
  query = seurat_georg.sc,
  normalization.method = "SCT",
  reference.reduction = "spca",
  dims = 1:50
)
...
```{r, fig.width=12, fig.height=6}
p1 = DimPlot(seurat_georg.sc, reduction = "ref.umap", group.by = "predicted.celltype.l1", label = TRUE, label.size = 3, repel = TRUE) + NoLegend()
p2 = DimPlot(seurat_georg.sc, reduction = "ref.umap", group.by = "predicted.celltype.l2", label = TRUE, label.size = 3 ,repel = TRUE) + NoLegend()

p1 + p2
```

```{r, fig.width=12, fig.height=3}
ggplot(seurat_georg.sc@meta.data, aes(x = predicted.celltype.l2, fill=predicted.celltype.l2))+
  geom_bar()+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
seurat_georg$predicted.celltype.l1 <- seurat_georg.sc$predicted.celltype.l1
seurat_georg$predicted.celltype.l1.score <- seurat_georg.sc$predicted.celltype.l1.score

seurat_georg$predicted.celltype.l2 <- seurat_georg.sc$predicted.celltype.l2
seurat_georg$predicted.celltype.l2.score <- seurat_georg.sc$predicted.celltype.l2.score
```

```{r, fig.height=4, fig.width=12}
ggplot(seurat_georg@meta.data, aes(x = as.character(patient_id), fill=predicted.celltype.l2))+
  geom_bar(position = "fill")+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
seurat_georg_Mono <- subset(seurat_georg, subset= predicted.celltype.l2 %in% c("CD14 Mono"))
seurat_georg_Mono
```
...
```{r}
## Define variable genes
seurat_georg_Mono <- FindVariableFeatures(object = seurat_georg_Mono,
                             assay="RNA",
                             selection.method = 'vst')

## Scaling
seurat_georg_Mono <- ScaleData(object = seurat_georg_Mono, assay = "RNA")

## PCA
seurat_georg_Mono <- RunPCA(object = seurat_georg_Mono, features = VariableFeatures(object = seurat_georg_Mono), 
                                   verbose = FALSE,
                                   assay="RNA")

## UMAP
seurat_georg_Mono <- RunUMAP(seurat_georg_Mono, reduction.use = "pca", dims = 1:20, seed.use = 42)
```


```{r, fig.width= 7, fig.height= 6}
DimPlot(object = seurat_georg_Mono,
        reduction = 'umap',
        group.by="disease_severity",
        label = TRUE) + scale_color_manual(values=c("control"="#4f4d4c",
                                                    "1-3" = "#ffb867",
                                                    "4-5"="#5D63A1",
                                                    "6-8"="#982326",
                                                    "FLI"="#27dabb",
...
```{r, fig.width=16,fig.height=20}
seurat_georg_Mono <- FindNeighbors(object = seurat_georg_Mono, dims = 1:10, reduction="pca", force.recalc = TRUE)
```

```{r}
seurat_georg_Mono <- FindClusters(object = seurat_georg_Mono, resolution = 0.2, algorithm = 1)
```
...
```{r, fig.height=5, fig.width=12}
ggplot(seurat_georg_Mono@meta.data, aes(x = RNA_snn_res.0.2, fill=as.character(patient_id)))+
  geom_bar(position="fill")+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
Idents(seurat_georg_Mono) <- "RNA_snn_res.0.2"
seurat_georg_Mono_cluster.markers.wilcox_res.0.2 <- FindAllMarkers(object = seurat_georg_Mono,
                                                                    only.pos = TRUE,
                                                                    min.pct = 0.2,
                                                                    logfc.threshold = 0.2,
                                                                    min.diff.pct = 0.1,
...
```{r, fig.width=8, fig.height=12}
top <- seurat_georg_Mono_cluster.markers.wilcox_res.0.2 %>% group_by(cluster) %>% top_n(n = 10, wt = abs(avg_log2FC))

p <- DotPlot(seurat_georg_Mono,
        group.by = "RNA_snn_res.0.2",
        features =c("CD14", unique(top$gene)))+
  scale_color_gradientn(colors = rev(RColorBrewer::brewer.pal(n =100, name = "RdBu")))+
  theme(axis.title = element_blank()) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
### Filter Monocyte/T cell cluster
```{r}
seurat_georg_Mono_filt <- subset(seurat_georg_Mono, subset = RNA_snn_res.0.2 == "2", invert=TRUE)
table(seurat_georg_Mono_filt$RNA_snn_res.0.2)
```
...
```{r}
# SaveH5Seurat(
#   object= seurat_georg_Mono_filt,
#   file= paste("seurat_objects/seurat_georg_Mono_filt_",Sys.Date(),sep=""))

# seurat_georg_Mono_filt <- LoadH5Seurat("seurat_objects/seurat_georg_Mono_filt_2022-12-07.h5seurat")
```

#-----------------------------------------------------------------------

# 10X 5', Su et al. Cell 2020

Data downloaded from https://www.sciencedirect.com/science/article/pii/S0092867420314446?via%3Dihub 

```{r}
seurat_su_orig <- LoadH5Seurat("seurat_objects/Su_Cell_2020_210324.h5seurat")
...
```{r}
head(seurat_su_orig@meta.data)
```


```{r}
seurat_su_orig$disease_stage_orig <- seurat_su_orig$disease_stage
seurat_su_orig$disease_stage <- ifelse(seurat_su_orig$diagnosis=="control","control",seurat_su_orig$disease_stage)
seurat_su_orig$disease_stage <- ifelse(is.na(seurat_su_orig$disease_stage),"unknown",seurat_su_orig$disease_stage)

table(seurat_su_orig$disease_stage)
table(seurat_su_orig$disease_stage,seurat_su_orig$diagnosis)
```


```{r}
seurat_su_orig$disease_severity <- ifelse(seurat_su_orig$diagnosis == "control", "control",
                                          ifelse(seurat_su_orig$Who.Ordinal.Scale %in% c("1","1 or 2","2", "3"), "1-3",
                                                 ifelse(as.character(seurat_su_orig$Who.Ordinal.Scale) %in% c(4,5), "4-5",
                                                        ifelse(as.character(seurat_su_orig$Who.Ordinal.Scale) %in% c(6,7), "6-8",
                                                               "error"))))

seurat_su_orig$disease_severity <- factor(seurat_su_orig$disease_severity, levels=c("control",
                                                                                    "1-3",
                                                                                    "4-5",
                                                                                    "6-8"))

table(seurat_su_orig$disease_severity,seurat_su_orig$disease_stage)
```

## Re-Processing

```{r}
seurat_su <- seurat_su_orig
DefaultAssay(seurat_su) <- "RNA"
```
...
#### Exclude timepoint T2

```{r}
seurat_su$Blood.draw.time.point <- ifelse(seurat_su$disease_severity == "control", "control", seurat_su$Blood.draw.time.point)

seurat_su <- subset(seurat_su, subset = Blood.draw.time.point %in% c("control","T1"))
seurat_su
```

```{r}
table(seurat_su$disease_severity,seurat_su$disease_stage)
```


```{r, fig.width= 24, fig.height=10}
p1<-VlnPlot(object = seurat_su, features = "nFeature_RNA",pt.size = 0, log =TRUE, group.by = "sample") + theme(legend.position="none") + ggtitle("Genes per Cell") + ylab("Number of Genes") + xlab("")+ geom_hline(yintercept=100, linetype="dashed", color = "red", size=1)+ 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

p2<-VlnPlot(object = seurat_su, features = "nCount_RNA", log =TRUE, pt.size = 0, group.by = "sample")+ theme(legend.position="none") + ggtitle("Transcripts per Cell") + ylab("Number of UMIs") + xlab("")+ geom_hline(yintercept=100, linetype="dashed", color = "red", size=1)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

seurat_su[["percent.mt"]] <- PercentageFeatureSet(object = seurat_su, pattern = "^MT-")
p3<-VlnPlot(object = seurat_su, features = "percent.mt",pt.size = 0, group.by = "sample")+ theme(legend.position="none") + ggtitle("Percent of MT- counts") + ylab("% of '^mt-' gene counts") + xlab("")+ geom_hline(yintercept=5, linetype="dashed", color = "red", size=1)+ 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
```{r, fig.height=6, fig.width=8}
## Normalization
seurat_su <- NormalizeData(object = seurat_su, normalization.method = "LogNormalize", scale.factor = 1e4)

## Define variable genes
seurat_su <- FindVariableFeatures(object = seurat_su,
                             assay="RNA",
                             selection.method = 'vst')

## Scaling
seurat_su <- ScaleData(object = seurat_su, assay = "RNA")

## PCA
seurat_su <- RunPCA(object = seurat_su, features = VariableFeatures(object = seurat_su), verbose = FALSE)

## UMAP
seurat_su <- RunUMAP(seurat_su, reduction.use = "pca", dims = 1:20, seed.use = 42)
```



```{r, fig.height=6, fig.width=8}
DimPlot(object = seurat_su,
        reduction = 'umap',
        label = FALSE,shuffle = T,
        group.by = "sample")
...
Normalize query data using sc transform 
```{r}
seurat_su.sc <- SCTransform(seurat_su, verbose = FALSE, assay = "RNA")
```
...
```{r, fig.width=12, fig.height=6}
p1 = DimPlot(seurat_su.sc, reduction = "ref.umap", group.by = "predicted.celltype.l1", label = TRUE, label.size = 3, repel = TRUE) + NoLegend()
p2 = DimPlot(seurat_su.sc, reduction = "ref.umap", group.by = "predicted.celltype.l2", label = TRUE, label.size = 3 ,repel = TRUE) + NoLegend()

p1 + p2
```

```{r, fig.width=12, fig.height=3}
ggplot(seurat_su.sc@meta.data, aes(x = predicted.celltype.l2, fill=predicted.celltype.l2))+
  geom_bar()+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
seurat_su$predicted.celltype.l1 <- seurat_su.sc$predicted.celltype.l1
seurat_su$predicted.celltype.l1.score <- seurat_su.sc$predicted.celltype.l1.score

seurat_su$predicted.celltype.l2 <- seurat_su.sc$predicted.celltype.l2
seurat_su$predicted.celltype.l2.score <- seurat_su.sc$predicted.celltype.l2.score
```

```{r, fig.height=4, fig.width=12}
ggplot(seurat_su@meta.data, aes(x = as.character(sample), fill=predicted.celltype.l2))+
  geom_bar(position = "fill")+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
seurat_su_Mono <- subset(seurat_su, subset= predicted.celltype.l2 %in% c("CD14 Mono"))
seurat_su_Mono
```
```{r}
table(seurat_su_Mono$disease_severity,seurat_su_Mono$disease_stage)
```
...
```{r}
## Define variable genes
seurat_su_Mono <- FindVariableFeatures(object = seurat_su_Mono,
                             assay="RNA",
                             selection.method = 'vst')

## Scaling
seurat_su_Mono <- ScaleData(object = seurat_su_Mono, assay = "RNA")

## PCA
seurat_su_Mono <- RunPCA(object = seurat_su_Mono, features = VariableFeatures(object = seurat_su_Mono), 
                                   verbose = FALSE,
                                   assay="RNA")

## UMAP
seurat_su_Mono <- RunUMAP(seurat_su_Mono, reduction.use = "pca", dims = 1:20, seed.use = 42)
```


```{r, fig.width= 7, fig.height= 6}
DimPlot(object = seurat_su_Mono,
        reduction = 'umap',
        group.by="disease_severity",
        label = TRUE) + scale_color_manual(values=c("control"="#4f4d4c",
                                                    "1-3" = "#ffb867",
...
```{r, fig.width=16,fig.height=20}
seurat_su_Mono <- FindNeighbors(object = seurat_su_Mono, dims = 1:10, reduction="pca", force.recalc = TRUE)
```

```{r}
seurat_su_Mono <- FindClusters(object = seurat_su_Mono, resolution = 0.2, algorithm = 1)
```
...
```{r}
Idents(seurat_su_Mono) <- "RNA_snn_res.0.2"
seurat_su_Mono_cluster.markers.wilcox_res.0.2 <- FindAllMarkers(object = seurat_su_Mono,
                                                                    only.pos = TRUE,
                                                                    min.pct = 0.2,
                                                                    logfc.threshold = 0.2,
                                                                    min.diff.pct = 0.1,
...
```{r, fig.width=8, fig.height=12}
top <- seurat_su_Mono_cluster.markers.wilcox_res.0.2 %>% group_by(cluster) %>% top_n(n = 10, wt = abs(avg_log2FC))

p <- DotPlot(seurat_su_Mono,
        group.by = "RNA_snn_res.0.2",
        features =c("CD14", unique(top$gene)))+
  scale_color_gradientn(colors = rev(RColorBrewer::brewer.pal(n =100, name = "RdBu")))+
  theme(axis.title = element_blank()) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
### Filter Monocyte/T cell cluster
```{r}
seurat_su_Mono_filt <- subset(seurat_su_Mono, subset = RNA_snn_res.0.2 == "3", invert=TRUE)
table(seurat_su_Mono_filt$RNA_snn_res.0.2)
```

```{r}
table(seurat_su_Mono_filt$disease_severity, seurat_su_Mono_filt$disease_stage)
```
...
```{r}
# SaveH5Seurat(
#   object= seurat_su_Mono_filt,
#   file= paste("seurat_objects/seurat_su_Mono_filt_",Sys.Date(),sep=""))

# seurat_su_Mono_filt <- LoadH5Seurat("seurat_objects/seurat_su_Mono_filt_2022-12-21.h5seurat")
```

# --------------------------------------------------------

# 10X 5', Van der Wijst et al. STM 2021

```{r}
seurat_wijst_orig <- readRDS("seurat_objects/Seurat_Wijst_2022_220515.rds")
...
```{r}
head(seurat_wijst_orig@meta.data)
```

```{r}
seurat_wijst_orig$sample <- paste(seurat_wijst_orig$donor, seurat_wijst_orig$timepoint, sep="_")
```

```{r}
table(seurat_wijst_orig$pulmonary_infection)
table(seurat_wijst_orig$non_pulmonary_infection)
```

```{r}
table(seurat_wijst_orig$timepoint, seurat_wijst_orig$onset_to_D0_days)
```

```{r}
table(seurat_wijst_orig$NIH_ordinal, seurat_wijst_orig$NIH_clinical)
```

```{r}
tmp <- unique(seurat_wijst_orig@meta.data[,c("sample", "donor", "timepoint","COVID_severity","death","NIH_ordinal","onset_to_D0_days","COVID_status")])
table(tmp$COVID_severity, tmp$onset_to_D0_days)
table(tmp$timepoint, tmp$NIH_ordinal)
table(tmp$timepoint, tmp$COVID_status)
table(tmp$timepoint, tmp$onset_to_D0_days)
table(tmp$onset_to_D0_days, tmp$COVID_status)
```


```{r}
seurat_wijst_orig$COVID_severity <- factor(seurat_wijst_orig$COVID_severity, levels = c("Healthy",
                                                                                        "Moderate_Negative",
                                                                                        "Moderate_Positive",
                                                                                        "Severe_Negative",
                                                                                        "Severe_Positive",
...
seurat_wijst_orig$timepoint <- factor(seurat_wijst_orig$timepoint, levels = c("None",
                                                                              "0",
                                                                              "4",
                                                                              "7",
                                                                              "14"))
...
```{r}
seurat_wijst_orig$disease_severity <- ifelse(as.character(seurat_wijst_orig$COVID_severity) == "Critical_Positive_anti-IFN", "7-8_anti-IFN",
                                             ifelse(as.character(seurat_wijst_orig$NIH_ordinal) %in% c(4,5), "4-5",
                                                    ifelse(as.character(seurat_wijst_orig$NIH_ordinal) %in% c(6,7), "6-8",
                                                           ifelse(as.character(seurat_wijst_orig$NIH_ordinal) == "Healthy", "control","error"))))

seurat_wijst_orig$disease_severity <- factor(seurat_wijst_orig$disease_severity, levels=c("control",
                                                                                          "4-5",
                                                                                          "6-8",
                                                                                          "7-8_anti-IFN"))
```
...
p1 <- DimPlot(object = seurat_wijst_orig, 
        reduction = 'umap', 
        label = FALSE, 
        group.by = "NIH_ordinal"
        )

p2 <- DimPlot(object = seurat_wijst_orig, 
        reduction = 'umap', 
        label = FALSE, 
        group.by = "cell_type"
        )

p3 <- DimPlot(object = seurat_wijst_orig, 
        reduction = 'umap', 
        label = FALSE, 
        group.by = "disease_severity"
...
```{r}
mtx <- GetAssayData(seurat_wijst_orig, assay="RNA", slot="counts")
mtx@Dimnames[[1]] <- as.character(seurat_wijst_orig@assays$RNA@meta.features$feature_name)

wijst_meta <- seurat_wijst_orig@meta.data[,c("donor",
                                             "timepoint",
                                             ...
                                             "original_leiden",
                                             "ct1","ct2", "ct3",
                                             "WBC_count1", "WBC_count2", "WBC_count3", 
...
seurat_wijst <- CreateSeuratObject(counts = mtx, 
                                   meta.data = wijst_meta)
```

### Filter for earliest sample per donor

```{r}
seurat_wijst <- subset(seurat_wijst, subset = timepoint %in% c("None","0"))

seurat_wijst
```

```{r}
tmp <- unique(seurat_wijst@meta.data[,c("sample", "disease_severity", "onset_to_D0_days")])
table(tmp$disease_severity, tmp$onset_to_D0_days)
```

```{r}
seurat_wijst$onset_to_D0_days <- ifelse(seurat_wijst$onset_to_D0_days == "nan",0,as.character(seurat_wijst$onset_to_D0_days))
table(seurat_wijst$onset_to_D0_days)

seurat_wijst$disease_stage <- ifelse(seurat_wijst$disease_severity == "control", "control",
                                     ifelse(seurat_wijst$onset_to_D0_days %in% c("blank","unknown"),"unknown",
                                            ifelse(seurat_wijst$onset_to_D0_days <= 10, "acute_early",
                                                   ifelse(seurat_wijst$onset_to_D0_days > 10, "acute_late",
                                                   "error"))))

table(seurat_wijst$onset_to_D0_days,seurat_wijst$disease_stage)
```

```{r}
tmp <- unique(seurat_wijst@meta.data[,c("sample", "disease_severity", "disease_stage")])
table(tmp$disease_severity, tmp$disease_stage)
```

### Quality Control
```{r, fig.width= 24, fig.height=10}
p1<-VlnPlot(object = seurat_wijst, features = "nFeature_RNA",pt.size = 0, log =TRUE, group.by = "sample") + theme(legend.position="none") + ggtitle("Genes per Cell") + ylab("Number of Genes") + xlab("")+ geom_hline(yintercept=100, linetype="dashed", color = "red", size=1)+ 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

p2<-VlnPlot(object = seurat_wijst, features = "nCount_RNA", log =TRUE, pt.size = 0, group.by = "sample")+ theme(legend.position="none") + ggtitle("Transcripts per Cell") + ylab("Number of UMIs") + xlab("")+ geom_hline(yintercept=100, linetype="dashed", color = "red", size=1)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

seurat_wijst[["percent.mt"]] <- PercentageFeatureSet(object = seurat_wijst, pattern = "^MT-")
p3<-VlnPlot(object = seurat_wijst, features = "percent.mt",pt.size = 0, group.by = "sample")+ theme(legend.position="none") + ggtitle("Percent of MT- counts") + ylab("% of '^mt-' gene counts") + xlab("")+ geom_hline(yintercept=5, linetype="dashed", color = "red", size=1)+ 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
```{r, fig.height=6, fig.width=8}
## Normalization
seurat_wijst <- NormalizeData(object = seurat_wijst, normalization.method = "LogNormalize", scale.factor = 1e4)

## Define variable genes
seurat_wijst <- FindVariableFeatures(object = seurat_wijst,
                             assay="RNA",
                             selection.method = 'vst')

## Scaling
seurat_wijst <- ScaleData(object = seurat_wijst, assay = "RNA")

## PCA
seurat_wijst <- RunPCA(object = seurat_wijst, features = VariableFeatures(object = seurat_wijst), verbose = FALSE)

## UMAP
seurat_wijst <- RunUMAP(seurat_wijst, reduction.use = "pca", dims = 1:20, seed.use = 42)
```


```{r, fig.width=24, fig.height=6}
p1 <- DimPlot(object = seurat_wijst, 
        reduction = 'umap', 
        label = FALSE, 
        group.by = "sample"
        )+NoLegend()

p2 <- DimPlot(object = seurat_wijst, 
        reduction = 'umap', 
        label = FALSE, 
        group.by = "cell_type"
        )

p3 <- DimPlot(object = seurat_wijst, 
        reduction = 'umap', 
        label = FALSE, 
        group.by = "disease_severity"
)+scale_color_manual(values=c("control"="grey",
...
p4 <- DimPlot(object = seurat_wijst, 
        reduction = 'umap', 
        label = FALSE, 
        group.by = "disease_stage"
)

CombinePlots(plots = list(p1,p2,p3,p4), ncol=4)
```
...
Normalize query data using sc transform 
```{r}
seurat_wijst.sc <- SCTransform(seurat_wijst, verbose = FALSE, assay = "RNA")
```

```{r}
anchors <- FindTransferAnchors(
  reference = reference,
  query = seurat_wijst.sc,
  normalization.method = "SCT",
  reference.reduction = "spca",
  dims = 1:50
)
...
```{r, fig.width=12, fig.height=6}
p1 = DimPlot(seurat_wijst.sc, reduction = "ref.umap", group.by = "predicted.celltype.l1", label = TRUE, label.size = 3, repel = TRUE) + NoLegend()
p2 = DimPlot(seurat_wijst.sc, reduction = "ref.umap", group.by = "predicted.celltype.l2", label = TRUE, label.size = 3 ,repel = TRUE) + NoLegend()

p1 + p2
```

```{r, fig.width=12, fig.height=3}
ggplot(seurat_wijst.sc@meta.data, aes(x = predicted.celltype.l2, fill=predicted.celltype.l2))+
  geom_bar()+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
seurat_wijst$predicted.celltype.l1 <- seurat_wijst.sc$predicted.celltype.l1
seurat_wijst$predicted.celltype.l1.score <- seurat_wijst.sc$predicted.celltype.l1.score

seurat_wijst$predicted.celltype.l2 <- seurat_wijst.sc$predicted.celltype.l2
seurat_wijst$predicted.celltype.l2.score <- seurat_wijst.sc$predicted.celltype.l2.score
```

```{r, fig.height=4, fig.width=12}
ggplot(seurat_wijst@meta.data, aes(x = as.character(sample), fill=predicted.celltype.l2))+
  geom_bar(position = "fill")+
  theme_classic()+
  theme(
    panel.grid=element_blank(),
...
```{r}
seurat_wijst_Mono <- subset(seurat_wijst, subset= predicted.celltype.l2 %in% c("CD14 Mono"))
seurat_wijst_Mono
```
...
```{r}
## Define variable genes
seurat_wijst_Mono <- FindVariableFeatures(object = seurat_wijst_Mono,
                             assay="RNA",
                             selection.method = 'vst')

## Scaling
seurat_wijst_Mono <- ScaleData(object = seurat_wijst_Mono, assay = "RNA")

## PCA
seurat_wijst_Mono <- RunPCA(object = seurat_wijst_Mono, features = VariableFeatures(object = seurat_wijst_Mono), 
                                   verbose = FALSE,
                                   assay="RNA")

## UMAP
seurat_wijst_Mono <- RunUMAP(seurat_wijst_Mono, reduction.use = "pca", dims = 1:20, seed.use = 42)
```


```{r, fig.width= 7, fig.height= 6}
DimPlot(object = seurat_wijst_Mono,
        reduction = 'umap',
        group.by="disease_severity",
        label = TRUE) + scale_color_manual(values=color_clusters) + NoLegend()
```
...
```{r, fig.width=16,fig.height=20}
seurat_wijst_Mono <- FindNeighbors(object = seurat_wijst_Mono, dims = 1:10, reduction="pca", force.recalc = TRUE)
```

```{r}
seurat_wijst_Mono <- FindClusters(object = seurat_wijst_Mono, resolution = 0.2, algorithm = 1)
```
...
```{r}
Idents(seurat_wijst_Mono) <- "RNA_snn_res.0.2"
seurat_wijst_Mono_cluster.markers.wilcox_res.0.2 <- FindAllMarkers(object = seurat_wijst_Mono,
                                                                    only.pos = TRUE,
                                                                    min.pct = 0.2,
                                                                    logfc.threshold = 0.2,
                                                                    min.diff.pct = 0.1,
...
```{r, fig.width=8, fig.height=12}
top <- seurat_wijst_Mono_cluster.markers.wilcox_res.0.2 %>% group_by(cluster) %>% top_n(n = 10, wt = abs(avg_log2FC))

p <- DotPlot(seurat_wijst_Mono,
        group.by = "RNA_snn_res.0.2",
        features =c("CD14", unique(top$gene)))+
  scale_color_gradientn(colors = rev(RColorBrewer::brewer.pal(n =100, name = "RdBu")))+
  theme(axis.title = element_blank()) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
### Filter Monocyte/T cell cluster
```{r}
seurat_wijst_Mono_filt <- subset(seurat_wijst_Mono, subset = RNA_snn_res.0.2 == "4", invert=TRUE)
table(seurat_wijst_Mono_filt$RNA_snn_res.0.2)
```
...
```{r}
# SaveH5Seurat(
#   object= seurat_wijst_Mono_filt,
#   file= paste("seurat_wijst_Mono_filt_",Sys.Date(),sep=""))

# seurat_wijst_Mono_filt <- LoadH5Seurat("seurat_objects/seurat_wijst_Mono_filt_2022-12-20.h5seurat")
```

#---
#---

## Merge Monocytes from Su et al., Cell 2020, van der Wijst, STM 2021, Georg et al., Cell 2021 and Sawitzki data

```{r}
seurat_georg_Mono_filt$disease_stage <- seurat_georg_Mono_filt$phase_full
seurat_georg_Mono_filt$disease_stage <- ifelse(seurat_georg_Mono_filt$disease_severity=="control", "control", seurat_georg_Mono_filt$disease_stage)

seurat_georg_tmp <- subset(seurat_georg_Mono_filt,
                           subset=disease_severity %in% c("control",
                                                          "1-3",
                                                          "4-5",
                                                          "6-8",
                                                          "7-8_anti-IFN")&
                             disease_stage %in% c("control",
                                                  "acute",
                                                  "acute_early",
                                                  "acute_late",
                                                  "unknown"))
...
```{r}
table(seurat_georg_tmp$disease_severity,seurat_georg_tmp$disease_stage)
```


```{r}
seurat_wijst_mono_counts <- GetAssayData(seurat_wijst_Mono_filt, assay="RNA", slot = "counts")
seurat_wijst_mono_counts <- seurat_wijst_mono_counts[!rowSums(seurat_wijst_mono_counts)==0,]

seurat_Sawitzki_mono_counts <- GetAssayData(seurat_Sawitzki_Mono_filt, assay="RNA", slot = "counts")
seurat_Sawitzki_mono_counts <- seurat_Sawitzki_mono_counts[!rowSums(seurat_Sawitzki_mono_counts)==0,]

seurat_georg_mono_counts <- GetAssayData(seurat_georg_tmp, assay="RNA", slot = "counts")
seurat_georg_mono_counts <- seurat_georg_mono_counts[!rowSums(seurat_georg_mono_counts)==0,]

seurat_su_mono_counts <- GetAssayData(seurat_su_Mono_filt, assay="RNA", slot = "counts")
seurat_su_mono_counts <- seurat_su_mono_counts[!rowSums(seurat_su_mono_counts)==0,]
```

```{r}
intersect_genes <- as.character(Reduce(intersect,list("wijst"=seurat_wijst_mono_counts@Dimnames[[1]],
                                                      "Sawitzki"=seurat_Sawitzki_mono_counts@Dimnames[[1]],
                                                      "georg"=seurat_georg_mono_counts@Dimnames[[1]],
                                                      "Su"=seurat_su_mono_counts@Dimnames[[1]])))
length(intersect_genes)
```


```{r}
seurat_wijst_Mono_filt[["RNAfilt"]] <- CreateAssayObject(counts = seurat_wijst_mono_counts[seurat_wijst_mono_counts@Dimnames[[1]] %in% intersect_genes,])
DefaultAssay(seurat_wijst_Mono_filt) <- "RNAfilt"

seurat_Sawitzki_Mono_filt[["RNAfilt"]] <- CreateAssayObject(counts = seurat_Sawitzki_mono_counts[seurat_Sawitzki_mono_counts@Dimnames[[1]] %in% intersect_genes,])
DefaultAssay(seurat_Sawitzki_Mono_filt) <- "RNAfilt"

seurat_georg_tmp[["RNAfilt"]] <- CreateAssayObject(counts = seurat_georg_mono_counts[seurat_georg_mono_counts@Dimnames[[1]] %in% intersect_genes,])
DefaultAssay(seurat_georg_tmp) <- "RNAfilt"

seurat_su_Mono_filt[["RNAfilt"]] <- CreateAssayObject(counts = seurat_su_mono_counts[seurat_su_mono_counts@Dimnames[[1]] %in% intersect_genes,])
DefaultAssay(seurat_su_Mono_filt) <- "RNAfilt"
```

```{r}
length(Reduce(intersect,list("wijst"=rownames(seurat_wijst_Mono_filt),
                             "Sawitzki"=rownames(seurat_Sawitzki_Mono_filt),
                             "georg"=rownames(seurat_georg_tmp),
                             "su"=rownames(seurat_su_Mono_filt))))
```


```{r}
seurat_wijst_Mono_filt$dataset <- "Wijst"
seurat_georg_tmp$dataset <- "Georg"
seurat_su_Mono_filt$dataset <- "Su"
seurat_Sawitzki_Mono_filt$dataset <- "Sawitzki"
```

```{r}
seurat_mono_merge <- merge(x=seurat_wijst_Mono_filt,
                           y=list(seurat_georg_tmp,
                                  seurat_Sawitzki_Mono_filt,
                                  seurat_su_Mono_filt))
...
```{r}
seurat_mono_merge@assays$RNA <- NULL
seurat_mono_merge@assays$HTO <- NULL
seurat_mono_merge@assays$integrated <- NULL
seurat_mono_merge@assays$AB <- NULL

seurat_mono_merge
```

```{r}
seurat_mono_merge$disease_stage <- ifelse(seurat_mono_merge$disease_stage == "acute_early", "early",
                                          ifelse(seurat_mono_merge$disease_stage == "acute_late", "late",
                                                 ifelse(seurat_mono_merge$disease_stage == "very late", "late",
                                                        seurat_mono_merge$disease_stage)))
```

```{r}
sum(is.na(seurat_mono_merge$disease_stage))
sum(is.na(seurat_mono_merge$disease_severity))
```

```{r}
table(seurat_mono_merge$dataset,seurat_mono_merge$disease_severity)
table(seurat_mono_merge$disease_severity,seurat_mono_merge$disease_stage)
```


```{r}
tmp <- unique(seurat_mono_merge@meta.data[,c("sample", "disease_severity","dataset")])

length(unique(seurat_mono_merge$sample))

table(tmp$disease_severity)
table(tmp$disease_severity, tmp$dataset)
```
...
```{r, fig.height=6, fig.width=8}
## Normalization
seurat_mono_merge <- NormalizeData(object = seurat_mono_merge, normalization.method = "LogNormalize", scale.factor = 1e4)

## Define variable genes
seurat_mono_merge <- FindVariableFeatures(object = seurat_mono_merge,
                             assay="RNAfilt",
                             selection.method = 'vst')

## Scaling
seurat_mono_merge <- ScaleData(object = seurat_mono_merge, assay = "RNAfilt")

## PCA
seurat_mono_merge <- RunPCA(object = seurat_mono_merge, features = VariableFeatures(object = seurat_mono_merge), verbose = FALSE)

## UMAP
seurat_mono_merge <- RunUMAP(seurat_mono_merge, reduction.use = "pca", dims = 1:20, seed.use = 42)

seurat_mono_merge$dataset <- factor(seurat_mono_merge$dataset, levels=c("Wijst", "Georg","Su", "Sawitzki"))

DimPlot(object = seurat_mono_merge,
        reduction = 'umap',
        label = FALSE,order=TRUE,
        group.by = "dataset")+
  scale_color_manual(values=c("Wijst"="#dd5129",
                              "Georg"="#0f7ba2",
                              "Su"="#B86092",
                              "Sawitzki"="#43b284"))
```
...
DimPlot(object = seurat_mono_merge,
        reduction = 'umap',
        label = FALSE,order=TRUE,
        group.by = "disease_severity")+ scale_color_manual(values= c("control"="#4f4d4c",
                                                                     "1-3" = "#ffb867",
                                                                     "4-5"="#5D63A1",
                                                                     "6-8"="#982326",
                                                                     "7-8_anti-IFN"="#DA2746")) 
...
```{r}
SaveH5Seurat(
  object= seurat_mono_merge,
  file= paste("seurat_objects/seurat_mono_merge_",Sys.Date(),sep=""))

# seurat_mono_merge <- LoadH5Seurat("seurat_objects/seurat_mono_merge_2023-01-12.h5seurat")
```

```{r}
seurat_mono_merge$dataset <- ifelse(seurat_mono_merge$dataset %in% c("RECAST","RECAST_new"),"Sawitzki",as.character(seurat_mono_merge$dataset))
```



### Marker Gene Expression (Suppl. Fig. 3B, bottom panel)

```{r, fig.width=18,fig.height=9}
seurat_mono_merge$disease_severity <- factor(seurat_mono_merge$disease_severity,levels=c("control",
                                                                                         "1-3",
                                                                                         "4-5",
                                                                                         "6-8",
                                                                                         "7-8_anti-IFN"))

p <- FeatureBoxPlot(seu=seurat_mono_merge,
                    by = "features", 
                    assay = "RNAfilt", 
                    slot = "data", 
                    exprs.function = "mean",
                    ...
                    select.idents = NULL, 
                    label.x = NA, pt.size = NA, facet.face = "bold.italic") + 
  ggtitle("Combined Data., Monocytes RNA expression")+
  theme(plot.title = element_text(size=12))+scale_shape_manual(values = c("control"=15,
                                                                          "early"=19,
                                                                          "late"=17,
                                                                          "unknown"=5))
...
```{r}
meta <- unique(seurat_mono_merge@meta.data[,c("sample","disease_severity","disease_stage","dataset")])
idx <- match(gsva.res.melt$sampleID, meta$sample)
gsva.res.melt$disease_severity <- meta$disease_severity[idx]
gsva.res.melt$disease_stage <- meta$disease_stage[idx]
gsva.res.melt$dataset <- meta$dataset[idx]

gsva.res.melt$disease_severity <- factor(gsva.res.melt$disease_severity, levels=c("control",
                                                                                  "1-3",
                                                                                  "4-5",
                                                                                  "6-8",
                                                                                  "7-8_anti-IFN"))
...
p <- ggplot(gsva.res.melt, aes(y =value, x = disease_severity)) +
  ...
  labs(color = "COVID19 severity", shape = "disease_stage")+
  scale_color_manual(values=c("control"="#4f4d4c",
                              "1-3" = "#ffb867",
                              "4-5"="#5D63A1",
                              "6-8"="#982326",
  ...
  ggpubr::stat_compare_means(mapping = aes_string(group_by), comparisons = list(c("1-3", "4-5"),
                                                                                c("1-3", "6-8"),
                                                                                c("1-3", "7-8_anti-IFN"),
                                                                                c("4-5", "6-8"),
                                                                                c("4-5", "7-8_anti-IFN"),
...
# Arunachalam et al. (Suppl. Fig 3A)
...
# Chua et al., Lung tissue data (Suppl. Fig 3F)
...
# Bernardes et al. 2020, Immunity (Suppl. Fig 3D/E)
...
```{r}
# load Kiel metadata
meta_Kiel <- read.csv("bernardes_2020/Clinicaldata_scRNAseq_kiel.csv", sep = ";")
...
# seurat_schulte_cohort2_CD8_filtVarFeatures <- LoadH5Seurat("seurat_objects/Seurat_Schulte-Schrepping_Rhapsody_CD8T_2022-08-17.h5seurat")
```
...
# seurat_schulte_cohort2_Mono <- LoadH5Seurat("seurat_objects/Seurat_Schulte-Schrepping_Rhapsody_CD14Mono_2022-08-17.h5seurat")
```
